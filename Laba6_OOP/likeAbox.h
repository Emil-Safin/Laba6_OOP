...
class likeAbox : public Figures {
protected:
	int xOrigin, yOrigin, x2, y2;
	int width, height;
public:
	likeAbox() :width(0), height(0) {}
	likeAbox(int x, int y, int w, int h) :width(w), height(h) {
		this->x = x;
		this->y = y;
		this->x2 = x;
		this->y2 = y;
	...
	likeAbox(int x, int y, int w, int h, Color color) :width(w), height(h) {
		this->x = x;
		this->y = y;
		this->x2 = x;
		this->y2 = y;
